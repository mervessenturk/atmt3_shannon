...
def get_args():
    ...
    parser.add_argument(
        '--warmup_strategy',
        choices=['none', 'linear', 'constant'],
        default='none',
        help='Learning rate warmup strategy.'
    )
    parser.add_argument(
        '--warmup_steps',
        type=int,
        default=4000,
        help='Number of optimizer steps over which to warm up the LR.'
    )
    parser.add_argument(
        '--warmup_lr_factor',
        type=float,
        default=0.1,
        help='For constant warmup: lr = base_lr * factor during warmup.'
    )
...
def main(args):
    ...
    def make_warmup_lambda(strategy, warmup_steps, factor):
        def lmbda(step):
            if strategy == "linear":
                if warmup_steps <= 0:
                    return 1.0
            ...
            if strategy == "constant":
                return factor if step < warmup_steps else 1.0
            return 1.0  # none
        return lmbda

    scheduler = LambdaLR(
        optimizer,
        lr_lambda=make_warmup_lambda(args.warmup_strategy, args.warmup_steps, args.warmup_lr_factor),
    )
